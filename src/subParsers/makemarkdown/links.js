  ...
  if (node.hasChildNodes() && node.hasAttribute('href')) {
    ...
    if (options.ghMentions && /(?:^| )user-mention\b/.test(classes)) {
      for (var ii = 0; ii < childrenLength; ++ii) {
        txt += showdown.subParser('makeMarkdown.node')(children[ii], options, globals, false, txt);
      }

    } else {
      txt = '[';
      for (var i = 0; i < childrenLength; ++i) {
        txt += showdown.subParser('makeMarkdown.node')(children[i], options, globals, false, txt);
      }
      txt += '](';
